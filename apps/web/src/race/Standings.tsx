...
export function Standings({ snapshot, heroId }: { snapshot: SessionSnapshot; heroId: string }) {
    ...
    <section className="glass-panel ds-standings">
      ...
      <div className="ds-stand-list">
        {ranked.map((c) => {
          const isHero = c.driverId === heroId;
          return (
            <div key={c.driverId} className={`ds-stand-row ${isHero ? "hero" : ""} ${c.inPits ? "pitting" : ""}`}>
              ...
              <span className="ds-stand-gap ds-mono">{isHero ? "" : formatGap(c.gapAhead ?? 0)}</span>
            </div>
          );
        })}
      </div>
