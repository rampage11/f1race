import type { RaceSnapshot } from "@f1race/race-engine";
      ...
      <div className="list">
        {snapshot.cars.map((c) => (
          <div key={c.driverId} className={`row ${c.driverId === heroId ? "hero" : ""} ${c.inPits ? "pitting" : ""}`}>
            ...
            <span className="tyre" style={{ color: c.tyreCompound === "hard" ? "#64748b" : TYRE_COLORS[c.tyreCompound] }}>
              {TYRE_LABEL[c.tyreCompound][0]}
            </span>
            <span className="gap">{c.driverId === heroId ? "" : formatGap(c.gapAhead)}</span>
          </div>
        ))}
      </div>
    </section>
