...
- `apps/web` — Vite + React + Canvas. Renders snapshots only. **No game logic belongs here** (online/anti-cheat invariant). Screen flow (Yandex-only hub/training): `landing → onboarding (SetupScreen mode="onboarding") → hub → race`, driven by a state machine in `App.tsx` keyed off `authProfile.heroConfirmed`. `apiBaseUrl()` (`src/identity.ts`) derives the **HTTP origin only** from `VITE_WS_URL` (which may carry a path — prod is `wss://f1-race.ru/ws`); it MUST strip the path or every API call lands on `/ws/...` and 404s. The WS connection uses `VITE_WS_URL` verbatim.
