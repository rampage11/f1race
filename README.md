...
7. Темп гонки фиксированный (выбор скорости убран из UI); пауза доступна в **solo**. В **multiplayer** (≥2 живых игроков) темп залочен в реальном времени, пауза отключена. Рендер машин — `requestAnimationFrame` с интерполяцией `sFraction` между снапшотами (буфер ~150 мс), так что плавный разгон/торможение виден глазом при ~10 снапшотах/сек.
...
Готово: Фазы 0–4 + Фаза 3-persistence + Yandex OAuth + **онбординг/хаб/тренировки/DriverRating (ТЗ «onboarding/progression»)** + **клиентская интерполяция рендера (P16, rAF-буфер ~150 мс)** — мультиплеер, лобби, матчмейкинг, сохранение прогресса, стартовый мини-гейм, фоновая прокачка навыков, двухфакторный рейтинг (вопрос Q5 `level` vs `skillSum` решён), плавный рендер машин.
- **Фаза 5 — кросс-платформа**: вынести `protocol.ts` в общий пакет, PWA/мобильный клиент, delta-snapshot (P7) для мобильных сетей.
