...
Браузерная аркадная игра по Формуле-1 (RPG + онлайн-аркада). **Онлайн-мультиплеер с матчмейкингом**: авторитетный сервер крутит симуляцию (квалификация → стартовый мини-гейм «светофор» → гонка) и раздаёт снапшоты клиентам по WebSocket; браузер только рендерит. Создание пилота, live-квалификация, лобби по дивизионам (добор ботами), пит-стопы, сохранение прогресса (XP/уровни), вход через Яндекс.
...
## Игровой цикл
1. Экран **«Создание пилота»** (или **«Войти через Яндекс»**): имя, страна, распределение 10 очков по 6 навыкам, команда, стартовая резина. У авторизованного игрока настройка сохраняется между сессиями.
2. **Лобби**: игрок попадает в очередь, сервер группирует по дивизионам (F4/F3/F2/F1, по уровню) и создаёт комнату (добор ботами до 20). Solo-игрок не ждёт > 2 с.
...
6. **Финиш**: XP за место/быстрейший круг/отыгранные позиции → рост уровня/дивизиона, сохраняется в БД.
...
```bash
pnpm --filter @f1race/race-engine test        # юнит-тесты движка (48)
pnpm --filter @f1race/server test             # WS-интеграция + Room + lobby + persistence + auth (82)
...
packages/race-engine/src
  ...
  formula.ts          темп, износ, шанс обгона, старт, xpForRace, levelFromXp, divisionForLevel
...
apps/server/src
  server.ts           HTTP (auth routes) + WebSocket, lifecycle
  room.ts             Room: N соединений ↔ драйверы, broadcast, XP/progression на финише
  lobby.ts            очередь + матчмейкинг-тик (по дивизионам, добор ботами)
  ...
  persistence/        DriverProfileRepository + SQLite impl (profiles, race_history)

apps/web/src
  App.tsx             переключение стадий (setup ↔ race), обработка /yandex-callback
  identity.ts         guest UUID + Yandex auth token/profile в localStorage
  race/SetupScreen.tsx   создание пилота + кнопка «Войти через Яндекс»
...
Готово: Фазы 0–4 + Фаза 3-persistence + Yandex OAuth (мультиплеер, лобби, матчмейкинг, сохранение прогресса, стартовый мини-гейм).
...
- **Фаза 6 — контент/RPG**: ещё 2–3 трассы, Q2/Q3 в квалификации, тренировки (idle-прогресс навыков → переоткрывает вопрос Q5: матчинг по `level` vs `skillSum`).
